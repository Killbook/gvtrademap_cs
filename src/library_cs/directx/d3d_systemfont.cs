/*-------------------------------------------------------------------------
 ...
 Direct3D
 システムフォント
 D3DXFontよりも高速だが、ASCII文字のみの描画となる
...
namespace directx
{
	...
	public class d3d_systemfont : IDisposable
	{
		...
		private d3d_sprite					m_sprite;			// 描画管理

		private int[]						m_width_tbl;		// 幅テーブル

		private Vector3						m_position;			// Puts()時の表示位置
		private float						m_return_x;			// 改行時のX位置
		...
		/*-------------------------------------------------------------------------
		 描画時のサイズを得る
		 リターンコードは無視されるため、縦は常に HEIGHT を返す
		---------------------------------------------------------------------------*/
