/*-------------------------------------------------------------------------
 ...
 Direct3D
 テクスチャ化されたフォント
 Fontは非常に遅いのでテクスチャにレンダリングしてキャッシュする
 テクスチャはFontがレンダリングすることで作られる
...
namespace directx
{
	...
	public class d3d_textured_font :IDisposable
	{
		...
		{
			private string					m_str;			// テクスチャ化された文字列
			private Vector2					m_size;			// 文字列のサイズ
															// 描画時に使用する
															// テクスチャサイズとは関係ない
			private Vector2					m_texture_size;	// テクスチャサイズ
			private Texture					m_texture;		// 描画用テクスチャ
			private int						m_ref_count;	// 参照回数
