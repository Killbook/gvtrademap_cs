...
namespace Utility.Xml
{
	...
	/// IniBaseのインターフェイスでのアクセスを実装するだけで設定の読み書きが行える。
	/// </summary>
	public class XmlIniSetting : IniSettingBase
	{
		...
		/// 構築
		/// </summary>
		/// <param name="file_name">読み込みファイル名(フルパスであること)</param>
		/// <param name="id">
		/// <para>設定ファイルが正しいことを認識するID</para>
	...
	public class XmlIni : IniBase
	{
		...
		/// <summary>
		/// 読み込みつつ作成
		/// 読み込みに失敗した場合は新しいDocumentを作成する
		/// </summary>
		/// <param name="file_name">読み込みファイル名</param>
		/// <param name="id">
		/// <para>設定ファイルが正しいことを認識するID</para>
		/// <para>アプリケーション毎にユニークなものにすること</para>
		/// </param>
		...
		/// 書き出し
		/// </summary>
		/// <param name="file_name">書き出し先ファイル名</param>
		...
		public override string GetProfile(string group_name, string name, string default_value)
		{
			...
			string		text	= get_first_text(node);		// 最初のXmlTextの内容を得る
		...
		public override string[] GetProfile(string group_name, string name, string[] default_value)
		{
			...
			foreach(XmlNode n in node.ChildNodes){
				string		text	= get_first_text(n);	// 最初のXmlTextの内容を得る
		...
		/// <summary>
		/// 編集するべきノードを得る。
		/// 情報更新用。ノードが無ければ作成して返す。
		/// ノードが既に存在していれば子供を全てを削除して返す。
		/// </summary>
